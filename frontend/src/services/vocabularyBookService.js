...
export default {
  ...
  // 向单词书添加单词（逐个）
  addWordToBook: (bookId, payload) => 
    request.post(`/api/vocabulary-books/${bookId}/words`, payload),

  // 从单词书删除单词
  removeWordFromBook: (bookId, wordId) => 
    request.delete(`/api/vocabulary-books/${bookId}/words/${encodeURIComponent(wordId)}`)
}